...
@section('title')
    {{ $settings['site_name'] ?? null }} || Product Details
@endsection
...
@section('content')
    ...
    <div class="container py-4">
        ...
        <form method="GET" action="{{ route('products.index') }}" class="row mb-3 g-2">
            <div class="col-md-3">
            ...
            @forelse($products as $product)
                <div class="col-md-4 mb-4">
                    <div class="card h-100">
                        <img src="{{ $product->thumb_image }}" class="card-img-top" alt="{{ $product->name }}">
                        <div class="card-body d-flex flex-column">
                            <h5>{{ $product->name }}</h5>
                            <p class="text-truncate">{{ $product->short_description }}</p>
                            <p class="fw-bold">₹{{ $product->price }}</p>
                            <a href="{{ route('product.show', $product->slug) }}" class="btn btn-primary mt-auto">View</a>
                        </div>
                    </div>
                </div>
            @empty
    ...
    <script>
        ...
        jQuery(function() {
            jQuery("#slider_range").flatslider({
                ...
                values: [{{ $from }}, {{ $to }}],
                range: true,
                einheit: '{{ $settings['currency_icon'] ?? NULL }}'
            });
        });
    </script>
@endpush
